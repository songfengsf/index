(function () {
    ...
    function on(curEle, evenType, evenFn) {
        if (/^zhufeng/.test(evenType)) {//->自定义属性的绑定方法
            ...
            ary.push(evenFn);
        } else {//->内置的，因为在用DOM2级事件绑定方法的时候属于浏览器在带的行为，浏览器的那套
                // 机制很好，不需要使用我们的自定义属性的绑定方法
    ...
    function fire(evenType, e) {//发布，通知的作用
